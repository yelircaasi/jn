...
def condition_from_tag(subdict: dict, key="tags") -> Callable[[dict], bool]:
    ...
    if "STRING" in subdict:
        tag = subdict["STRING"]
        tag_condition =  (lambda d: match_empty(d[key]) or modify(tag in d[key])) \
            if optional \
            else lambda d: modify(tag in d[key])
    elif "REGEX" in subdict:
        reg = re.compile(subdict["REGEX"])
        found = lambda tag: re.search(reg, tag)
        tag_condition = (lambda d: match_empty(d[key]) or modify(any(map(found, d[key])))) \
            if optional \
